/* ================================================
   MERI DUKAAN v7.0 — AUTH
   Google Login · PIN · Navigation
   Real-time Listeners · Date Picker · Customer Picker
   Firestore Helpers

   PHASE 1 FIXES:
   ✅ setupListeners — all listeners use requestDashRefresh()
      instead of calling refreshDash() directly. Prevents the
      "Today's Activity appears 3 times" rendering bug.
   ✅ verifyPin — upgraded to SHA-256, with seamless v1→v2
      migration. Users with old btoa PINs are auto-upgraded
      on their next successful login.
   ✅ initConfirm — new PINs are always stored as SHA-256 hash
   ✅ pinAttempts / pinLockUntil — now persisted in localStorage
      so brute-force protection survives page refreshes
   ================================================ */
...
function googleSignIn() {
    ...
    auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL).then(function() {
        ...
        btn.disabled = false;
        btn.querySelector('span').textContent = 'Sign in with Google';
        showToast('❌ Sign-in failed in incognito mode. Use a normal browser window.', 'error');
    });
}
...
function signOutApp() {
    showConfirm('🚪', 'Sign Out?', 'You will be logged out from this device.', function() {
        ...
        auth.signOut().then(function() {
            ...
            allWaste = []; allCreditPayments = []; allNotes = [];
            goTo('loginScreen');
...
async function handleAuthenticated(user) {
    ...
    try {
        ...
        if (!ownerSnap.empty) {
            ...
            var memberSnap;
            try {
                memberSnap = await fdb.collection('businesses')
        ...
        if (!bizData.pin) {
            ...
            // Cache pin hash + version for offline use
            localStorage.setItem('mdPin', bizData.pin);
            localStorage.setItem('mdPinVersion', bizData.pinVersion || 'v1');
            goTo('pinLoginScreen');
        ...
        if (cachedBizId && cachedPin) {
            ...
            if (pu2) pu2.innerHTML = '<span>' + esc(user.email) +
                '</span><span style="color:#ff8a80;font-size:10px;margin-left:4px">Offline</span>';
            showToast('📴 Working in offline mode', 'warning');
        } else {
            showToast('❌ Internet required for first-time setup.', 'error');
...
function setupListeners() {
    ...
    unsubscribers.push(
        businessRef.collection('sales').onSnapshot(function(snap) {
            ...
            // FIX: use requestDashRefresh() — debounced, prevents rapid multi-call render bug
            requestDashRefresh();
    ...
    unsubscribers.push(
        businessRef.collection('creditPayments').onSnapshot(function(snap) {
            ...
            // FIX: use requestDashRefresh()
            requestDashRefresh();
        }, function(err) { console.error('[Sync] CreditPayments:', err); })
    );
...
// ---- PIN Setup (Step 1: enter new PIN) ----
function initSetup() {
    pinIn = '';
    setDots('setupDots', 0);
    var er = document.getElementById('setupErr');
    ...
    buildPad('setupPad',
        ...
        function() {
            if (pinIn.length > 0) { pinIn = pinIn.slice(0, -1); setDots('setupDots', pinIn.length); }
        }
    );
}

// ---- PIN Confirm (Step 2: confirm new PIN → hash → save) ----
function initConfirm() {
    pinIn = '';
    setDots('confirmDots', 0);
    var er = document.getElementById('confirmErr');
    if (er) er.textContent = '';
    buildPad('confirmPad',
        function(d) {
            if (pinIn.length < 4) {
                ...
                if (pinIn.length === 4) {
                    if (pinIn === pin1) {
                        // FIX: Hash with SHA-256 before saving (was btoa — just base64)
                        var enteredPin = pinIn;
                        pinIn = '';
                        var salt = businessId || 'default';
                        hashPin(enteredPin, salt).then(function(hash) {
                            return businessRef.update({ pin: hash, pinVersion: 'v2' })
                                .then(function() { return hash; });
                        }).then(function(hash) {
                            localStorage.setItem('mdPin', hash);
                            localStorage.setItem('mdPinVersion', 'v2');
                            pin1 = '';
                            showToast('✅ PIN set successfully!');
                            setTimeout(function() { goTo('dashboardScreen'); }, 300);
                        }).catch(function(err) {
                            console.error('[PIN Setup]', err);
                            showToast('❌ Error saving PIN. Please retry.', 'error');
                        });
                    } else {
                        pinIn = '';
                        pinErr('confirmDots', 'confirmErr', 'PINs do not match!');
        ...
        function() {
            if (pinIn.length > 0) { pinIn = pinIn.slice(0, -1); setDots('confirmDots', pinIn.length); }
        }
    );
}
...
function initLogin() {
    ...
    // Check if still locked (survives page refresh now)
    if (Date.now() < pinLockUntil) {
        var remSec = Math.ceil((pinLockUntil - Date.now()) / 1000);
        if (er) er.textContent = '🔒 Too many attempts. Wait ' + remSec + 's.';
    }
    ...
    buildPad('loginPad',
        ...
        function() {
            if (pinIn.length > 0) { pinIn = pinIn.slice(0, -1); setDots('loginDots', pinIn.length); }
        }
    );
}

/**
 * verifyPin — Phase 1 rewrite
 *
 * Supports two PIN storage versions:
 *   v1 (legacy): btoa(pin)          — Base64, insecure, auto-migrates to v2 on success
 *   v2 (current): SHA-256(pin+salt) — Cryptographic hash, secure
 *
 * Migration is seamless: user logs in normally, and the next time they open
 * the app their PIN is stored as SHA-256. No user action required.
 */
async function verifyPin(entered) {
    try {
        var stored, version;

        // Try Firestore first (most up-to-date), fallback to localStorage (offline)
        if (businessRef) {
            try {
                var doc = await businessRef.get();
                if (doc.exists) {
                    stored  = doc.data().pin;
                    version = doc.data().pinVersion || 'v1';
                } else {
                    stored  = localStorage.getItem('mdPin');
                    version = localStorage.getItem('mdPinVersion') || 'v1';
                }
            } catch (fetchErr) {
                stored  = localStorage.getItem('mdPin');
                version = localStorage.getItem('mdPinVersion') || 'v1';
        ...
        if (version === 'v2') {
            ...
            var hash = await hashPin(entered, salt);
            isMatch  = (!!entered && hash === stored);
        } else {
            // v1 legacy: compare with btoa-decoded stored PIN
            var sv = '';
            try { sv = atob(stored || ''); } catch (e) { /* invalid base64 */ }
            isMatch = (entered === sv);

            // Auto-migrate to v2 on successful v1 login
            if (isMatch) {
                var newHash = await hashPin(entered, salt);
                // Update Firestore + localStorage silently in background
                var updateData = { pin: newHash, pinVersion: 'v2' };
                if (businessRef) {
                    businessRef.update(updateData).catch(function(e) {
                        console.warn('[PIN] Migration to v2 failed:', e.message);
                    });
                }
                localStorage.setItem('mdPin', newHash);
                localStorage.setItem('mdPinVersion', 'v2');
                console.log('[PIN] Migrated from v1 (base64) → v2 (SHA-256)');
            }
        }
        ...
        if (isMatch) {
            ...
            pinAttempts  = 0;
            pinLockUntil = 0;
            localStorage.setItem('mdPinAttempts',  '0');
            localStorage.setItem('mdPinLockUntil', '0');
            setTimeout(function() { goTo('dashboardScreen'); }, 200);
        } else {
            // Failure
            pinIn = '';
            pinAttempts++;
            localStorage.setItem('mdPinAttempts', pinAttempts.toString());

            if (pinAttempts >= 5) {
                var lockTime = Date.now() + 30000; // 30-second lockout
                pinLockUntil = lockTime;
                localStorage.setItem('mdPinLockUntil', lockTime.toString());
                localStorage.setItem('mdPinAttempts',  '0');
                pinAttempts = 0;
                pinErr('loginDots', 'loginErr', '🔒 Too many attempts! Wait 30s.');
            } else {
                pinErr('loginDots', 'loginErr', 'Incorrect PIN! (' + (5 - pinAttempts) + ' attempts left)');
            }
        }
    } catch (err) {
        console.error('[PIN] Verify error:', err);
        pinIn = '';
        showToast('❌ PIN verification error. Please retry.', 'error');
    }
}
...
function goTo(id) {
    ...
    switch (id) {
        ...
        case 'wasteScreen':
            ...
            break;
        case 'creditScreen':
            var crpD = document.getElementById('crpDate');
            if (crpD) crpD.max = todayStr();
...
function dpMonth(off) {
    var nd  = new Date(dpViewDate);
    nd.setMonth(nd.getMonth() + off);
    var now = new Date();
    if (nd.getFullYear() > now.getFullYear() ||
        (nd.getFullYear() === now.getFullYear() && nd.getMonth() > now.getMonth())) return;
    dpViewDate = nd;
    renderCalendar();
}
function renderCalendar() {
    ...
    var todayS = todayStr(); var h = '';
    for (var e = 0; e < first; e++) h += '<button class="dp-day empty" aria-hidden="true"></button>';
    ...
    if (nextBtn) nextBtn.disabled =
        (dpViewDate.getFullYear() >= now2.getFullYear() &&
...
// ============ APP START ============
function startApp() {
    console.log('🫓 Meri Dukaan v7.0 Starting...');
    applyTheme();
    ...
    auth.getRedirectResult()
        .then(function(result) {
            // Redirect result handled by onAuthStateChanged below
            if (result && result.user) console.log('[Auth] Redirect sign-in successful.');
        })
        .catch(function(err) {
            if (err.code && err.code !== 'auth/popup-closed-by-user') {
                console.warn('[Auth] Redirect error:', err.message || err);
            }
        });
...
console.log('%c🫓 Meri Dukaan v7.0 %c Phase 1 — Stabilized',
